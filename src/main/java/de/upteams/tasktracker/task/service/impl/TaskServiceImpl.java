...
public class TaskServiceImpl implements TaskService {
    ...
    @Override
    @Transactional
    public void update(final TaskDto dto, final AppUser changer) {
        final Task existedTask = getOrThrow(dto.getId());
        ...
        final String newTitle = dto.getTitle();
        final String newDescription = dto.getDescription();

        if (!StringUtils.isBlank(newTitle)) {
            existedTask.setTitle(newTitle);
        }

        if (!StringUtils.isBlank(newDescription)) {
            existedTask.setDescription(newDescription);
        }
    }
